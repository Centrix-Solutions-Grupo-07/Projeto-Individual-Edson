...
pip3 install mysql-connector-python pymssql nltk beautifulsoup4 selenium
...
sudo docker exec -it ContainerBD bash -c "mysql -u root -p'urubu100' -e 'CREATE DATABASE IF NOT EXISTS centrix; USE centrix; CREATE TABLE IF NOT EXISTS grosseria (palavraGrossa VARCHAR(45) PRIMARY KEY,quantidadeTotalAtual INT,dataMonitoracaoAtual DATETIME,quantidadeTotalPassado INT,dataMonitoracaoPassado DATETIME); CREATE TABLE IF NOT EXISTS Login (idLogin INT primary key auto_increment, Email VARCHAR(45), Atividade VARCHAR(255), Id_do_dispositivo CHAR(16), dataHoraEntrada DATETIME, dataHoraSaida DATETIME);'"

echo -e "\n[Centrix Bot]: Clonando o repositório com o JAR..."
git clone  https://github.com/Centrix-Solutions-Grupo-07/Projeto_Individual_Edson.git
